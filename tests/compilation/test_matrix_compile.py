#!/usr/bin/env python
"""
Simple test script for matrix library compilation
"""

import pyopencl as cl
import sys

# OpenCL kernel source
kernel_src = """
// Define fpreal types
#ifndef fpreal
#define fpreal float
#endif

typedef fpreal fpreal2 __attribute__((ext_vector_type(2)));
typedef fpreal fpreal3 __attribute__((ext_vector_type(3)));
typedef fpreal fpreal4 __attribute__((ext_vector_type(4)));
typedef fpreal fpreal16 __attribute__((ext_vector_type(16)));

// Define old Houdini matrix types (for compatibility)
typedef fpreal3 mat3[3];
typedef fpreal4 mat2;
typedef fpreal16 mat4;

// Include new matrix library
#include "matrix_types.h"
#include "matrix_ops.h"

// Test kernel
kernel void test_matrix_operations(global float* output) {
    int idx = 0;

    // Test matrix3x3 diagonal constructor
    matrix3x3 m3_diag = GLSL_matrix3x3_diagonal(3.0f);
    output[idx++] = m3_diag.cols[0].x;

    // Test full constructor
    matrix3x3 m3_full = GLSL_mat3(
        1.0f, 2.0f, 3.0f,
        4.0f, 5.0f, 6.0f,
        7.0f, 8.0f, 9.0f
    );
    output[idx++] = m3_full.cols[0].x;

    // Test matrix-vector multiplication
    float3 v = (float3)(1.0f, 2.0f, 3.0f);
    float3 result_mv = GLSL_mul_mat3_vec3(m3_full, v);
    output[idx++] = result_mv.x;

    // Test transpose
    matrix3x3 m3_t = GLSL_transpose_mat3(m3_full);
    output[idx++] = m3_t.cols[0].x;

    // Test matrix2x2
    matrix2x2 m2 = GLSL_mat2(1.0f, 2.0f, 3.0f, 4.0f);
    output[idx++] = m2.cols[0].x;

    // Test determinant
    float det2 = GLSL_determinant(m2);
    output[idx++] = det2;

    // Test inverse
    matrix2x2 m2_inv = GLSL_inverse(m2);
    output[idx++] = m2_inv.cols[0].x;

    // Test matrix4x4
    matrix4x4 m4_diag = GLSL_matrix4x4_diagonal(2.0f);
    output[idx++] = m4_diag.cols[0].x;
}
"""

def main():
    # Create OpenCL context
    platforms = cl.get_platforms()
    if not platforms:
        print("ERROR: No OpenCL platforms found")
        return 1

    ctx = cl.Context(
        dev_type=cl.device_type.ALL,
        properties=[(cl.context_properties.PLATFORM, platforms[0])]
    )

    # Build options
    build_opts = "-I ../../houdini/ocl/include"

    print("=" * 70)
    print("Matrix Library Compilation Test")
    print("=" * 70)
    print(f"Platform: {platforms[0].name}")
    print(f"Device: {ctx.devices[0].name}")
    print(f"Build options: {build_opts}")
    print("=" * 70)
    print("Compiling...")

    try:
        program = cl.Program(ctx, kernel_src).build(options=build_opts)
        print("=" * 70)
        print("SUCCESS: Matrix library compiled successfully!")
        print("=" * 70)
        return 0
    except Exception as e:
        print("=" * 70)
        print("COMPILATION FAILED:")
        print("=" * 70)
        print(str(e))
        return 1

if __name__ == "__main__":
    sys.exit(main())
